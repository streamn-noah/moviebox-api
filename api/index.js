...
function isAuthorized(request, env2) {
  ...
  return !!secret && secret === env2.MOVIEBOX_SECRET;
}
var LANGUAGE_NAMES = {
  en: "English",
  fr: "Fran\xE7ais",
...
async function handleInfo(subjectId, env2) {
  const data = await fetchWithHostPool(
    env2,
    PATHS.get,
    "GET",
...
async function handleSeason(subjectId, env2) {
  const data = await fetchWithHostPool(
    env2,
    PATHS.seasonInfo,
    "GET",
...
async function handleStreamAll(subjectId, env2) {
...
async function handleDownload(subjectId, env2) {
  const pack = await fetchResourcePack(env2, subjectId);
...
async function handleHomeRows(env2) {
  const data = await fetchH5Home(env2.NIGERIA_IP);
...
async function handleHome(env2) {
  const data = await fetchH5Home(env2.NIGERIA_IP);
  if (!data) return err("Failed to fetch homepage", 502);
  const rows = (data.operatingList || []).map((row) => ({
    title: row.title,
...
async function handleHomeSubjects(opId, env2) {
  const data = await fetchH5Home(env2.NIGERIA_IP);
  if (!data) return err("Failed to fetch homepage", 502);
...
if (process.env.PROXY_URL) {
  try {
    ...
    console.error("Proxy setup failed:", err2.message);
  }
}
var memKV = {};
var KV_PATH = "/tmp/vercel_kv.json";
function readDiskKV() {
  try {
...
function writeDiskKV(db) {
  try {
    fs.writeFileSync(KV_PATH, JSON.stringify(db), "utf8");
  } catch {
  }
}
var kvMock = {
...
async function handler(req, res) {
  try {
    ...
    const webRes = await index_default.fetch(webReq, env);
    ...
    console.error("Handler error:", err2);
    res.statusCode = 500;
